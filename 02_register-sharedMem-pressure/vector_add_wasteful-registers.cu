...
__global__ void vector_add_reg_heavy(const float* __restrict__ a,
...
void run_vector_add(unsigned int n, unsigned int block_size){
    ...
    vector_add_reg_heavy<<<grid, block>>>(d_a, d_b, d_c, n);
...
int main(){
    ...
    unsigned int block_size = getenv("BLOCK_SIZE") ? atoi(getenv("BLOCK_SIZE")) : 128;
